...
For purposes of illustration, assume that each class is equally likely *a priori*: $P(c = c_{sub}) = 0.5$.
...
hence, listeners who expect speakers to be informative will prefer lower variance (e.g., subordinate) comparison classes (Prediction 2).
...
In Expt. 2, we gather judgments about adjectives when the comparison class is explicit: specifically, whether or not an adjective would apply to a subordinate member explicitly relative to the superordinate category (e.g., Is a day in winter warm relative to other days of the year?).
