...
[] why does the model make the predictions that it does?
[] why experiment 2?
...
To our knowledge, this problem central to language understanding has received no attention. 
...
For illustration, consider temperature once again. 
The average temperature in Winter is likely to be lower than the average for the year. 
Saying "it's warm" (a *positive* form adjective) in Winter should signal it's warm for Winter (the more specific class), because it's unlikely to be true that it's warm for the year (the more general class). 
The opposite should be true in Summer, where warm should be more likely to signal the general class. 
In addition, this pattern should reverse for *negative* form adjectives (e.g., "cold"), again due to standard Bayesian reasoning of what is more likely to be true (Prediction 1). 
In addition, the amount of information conveyed by a vague utterance depends upon the comparison class. 
Higher variance comparison classes will result in relatively less information gain by the listener, and hence, listeners who expect speakers to be informative will prefer lower variance (e.g., more specific) comparison classes. 
For example, regardless of the season, a listener should prefer "warm" or "cold" to be relative to the current season, a more specific reference class than "the whole year" (Prediction 2). 
...
In this paper, we pursue a different methodology.
Our model captures a productive fragment of natural language; thus, it makes predictions about related natural language tasks.
Critically, we can use the model to predict natural language judgments *based on the same prior knowledge* and use Bayesian data analysis to jointly infer the prior knowledge. 
In Expt. 2 we collect judgments about adjective sentences like "This [winter day] is cold relative to other days of the year," in which the same domain knowledge is needed.
...
When a listener hears only that "it's warm outside" without an explicit comparison class (e.g., "warm for the season"), we posit the listener must infer the comparison class by thinking about continuations that would best complete the sentence.
She does this using her world knowledge of what worlds are plausible given different comparison classes $P(x \mid c)$, what comparison classes are likely to be talked about $P(c)$, and how a rational speaker would behave in a given world and comparison class $S_{1}(u \mid x, c, \theta)$ (Eq. \ref{eq:L1a}).
...
Crucially in this situation, the listener is aware that the target entity is in the subordinate class (e.g., aware that it is winter) and draws likely values of the degree (e.g., temperature) from the subordinate class prior $P(x \mid c_{sub})$. With these assumptions, the model becomes:
\vspace{-0.5cm}
\begin{align}
...
As described in the introduction, Figure \ref{fig:modelSchematics} (right) shows that during Winter, the pragmatic listener upon hearing "it's warm" (positive adjective) thinks it's more likely that it's warm relative to days in Winter than when she hears "it's cold" (Figure \ref{fig:modelSchematics}, right, left-most bars).
The reverse is true in Summer (Prediction 1).
...
The pragmatic listener (Eq. \ref{eq:L1a}) prefers the more specific comparison class (Prediction 2).
...
In Expt. 2, we gather judgments about adjective usage in contexts where the comparison class is explicit: whether or not an adjective would be true relative to the superordinate category of a subordinate member (e.g., it is a winter day, is it warm relative to other days of the year?).

To model this this data, we remove comparison class uncertainty by setting $P(c_{super}) = 1$, since the sentences provide explicit comparison classes.
...
Experiment 2 collects further data about adjective usage in order to constrain the quantitative predictions for the RSA model, which will be used to predict data from both experiments.
